...
pub fn sys_get_time(ts: *mut TimeVal, _tz: usize) -> isize {
    ...
    let ret_slice =
        unsafe { core::slice::from_raw_parts(&ret as *const TimeVal as *const u8, tot_len) };
    let buffers = translated_byte_buffer(current_user_token(), ts as *const u8, tot_len);
    let mut offset = 0;
    for buf in buffers {
        buf.copy_from_slice(&ret_slice[offset..]);
        offset += buf.len();
    }
    0
}
